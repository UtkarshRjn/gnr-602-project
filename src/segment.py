...
class Segment:
    def __init__(self, dr_algo: str, classifier_algo: str):
    ...
    def fit(self, data: np.ndarray, labels: np.ndarray, num_components: int, num_epochs: int = 50, batch_size: int = 32, validation_split: float = 0.2):
        ...
        if self.dim_red_algo == 'pca':
            ...
            data_reduced, lda_components = self.lda(data_scaled, labels.ravel(), num_components)
            with open('../models/lda_model.pkl', 'wb') as f:
                pickle.dump(lda_components, f)
        else:
            return None
        
        save_fig(data_reduced[:,0].reshape(n,m), name='lda_1.png', dir='../output/lda/', keep_axis=False)
        save_fig(data_reduced[:,1].reshape(n,m), name='lda_2.png', dir='../output/lda/', keep_axis=False)
        save_fig(data_reduced[:,2].reshape(n,m), name='lda_3.png', dir='../output/lda/', keep_axis=False)
        ...
        X_train, X_test, y_train, y_test = train_test_split(data_reduced, labels_onehot, test_size=0.3, random_state=42)

        # Train the neural network on the training set
        self.model.fit(X_train, y_train, epochs=num_epochs, batch_size=batch_size, validation_split=validation_split)

        # Evaluate the neural network on the test set
        loss, accuracy = self.model.evaluate(X_test, y_test)
        print("Test accuracy:", accuracy)

        self.model.save('../models/' +  self.dim_red_algo + '_' + self.classifier_algo + '.h5')
    ...
    def predict(self, image: np.ndarray, gauss: bool = True, num_components: int = 16):
        ...
        self.model = load_model('../models/' + self.dim_red_algo + '_' + self.classifier_algo + '.h5')
        
        if self.dim_red_algo == 'pca':
            reduced_image =  self.pca(scaled_image, num_components)
        elif self.dim_red_algo == 'lda':
            lda_components = pickle.load(open('../models/lda_model.pkl', 'rb'))
